 ...
 * @returns 是否成功发现市场
 */
export async function dynamicDiscoverMarket(
  db: Database.Database,
  conditionId: string
): Promise<boolean> {
  ...
  // 处理并保存市场（不关联到事件）
  try {
    await processMarket(db, market, null as any);
 ...
 * @returns 总共发现的市场数量
 */
export async function discoverMultipleEvents(
  db: Database.Database,
  eventSlugs: string[]
): Promise<number> {
  let totalMarkets = 0;
