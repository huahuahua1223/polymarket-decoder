...
async function fetchJson(url) {
    ...
    if (!response.ok) {
        throw new Error(`Gamma API 请求失败: ${response.status} ${response.statusText} - ${url}`);
    }
 ...
 * ```
 */
export async function getEventMarkets(eventSlug) {
 ...
 * ```
 */
export async function getAllMarkets(limit) {
 ...
 * @returns Oracle 地址
 */
export function extractOracleAddress(market) {
 ...
 * @returns 是否有效
 */
export function validateGammaMarket(market) {
