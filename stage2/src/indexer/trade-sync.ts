...
import {
  ...
  ORDER_FILLED_ABI,
  USDC_DECIMALS,
} from '../../../stage1/src/constants/index.js';
import { decodeEventLog, formatUnits, type Hex, type Log } from 'viem';
import type { OrderFilledEvent } from '../../../stage1/src/types/contracts.js';
import * as store from '../db/store.js';
import { dynamicDiscoverMarket } from './market-discovery.js';
import { retryWithBackoff } from '../utils/retry.js';
import type { SyncResult, BlockInfo, TradeInsert } from '../types/index.js';

// 区块信息缓存（避免重复查询）
const blockCache = new Map<bigint, BlockInfo>();

/**
 * 获取区块信息（带缓存）
 */
async function getBlockInfo(blockNumber: bigint): Promise<BlockInfo> {
 ...
 * 解码单个 OrderFilled 日志
 */
function decodeOrderFilledLog(log: Log, txHash: string): {
  maker: string;
  taker: string;
  makerAssetId: bigint;
  takerAssetId: bigint;
...
function calculateSize(
  ...
  // tokenAmount 是头寸的数量
  const tokenAmount =
    makerAssetId === 0n ? takerAmountFilled : makerAmountFilled;
...
async function processTradeLogs(
  ...
  for (const log of logs) {
    try {
      ...
      // 确定 outcome (YES/NO)
      const outcome =
        decoded.tokenId.toLowerCase() === market.yes_token_id.toLowerCase()
          ? 'YES'
          : 'NO';
      ...
      console.error(
        `  处理日志失败 (tx: ${log.transactionHash}, logIndex: ${log.logIndex}):`,
        error
      );
    }
  }
...
export async function runIndexer(
  ...
  while (currentBatchStart <= endBlock) {
    const batchEnd =
      currentBatchStart + BATCH_SIZE - 1n > endBlock
        ? endBlock
    ...
    try {
      ...
      const logs = await retryWithBackoff(
        () => fetchOrderFilledLogs(currentBatchStart, batchEnd),
        3,
        2000
      );
