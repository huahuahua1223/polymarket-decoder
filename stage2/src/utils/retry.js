...
export async function retryWithBackoff(fn, maxRetries = 5, initialDelay = 1000) {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            console.warn(`重试 ${attempt + 1}/${maxRetries}，等待 ${delay}ms (错误: ${lastError.message})`);
            await sleep(delay);
        }
    }
 ...
 * @returns Promise 结果
 */
export async function withTimeout(promise, timeoutMs, errorMessage = '操作超时') {
    let timeoutHandle;
 ...
 * @returns 所有任务结果
 */
export async function batchExecute(tasks, fn, concurrency = 5, onProgress) {
    const results = [];
    const executing = [];
    let completed = 0;
