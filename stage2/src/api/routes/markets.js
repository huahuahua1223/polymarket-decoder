...
export const marketsRoutes = async (fastify) => {
    const db = fastify.db;
    /**
     * GET /markets/:slug
     * 获取市场详情
     */
    fastify.get('/markets/:slug', {
        schema: {
            params: {
                type: 'object',
                properties: {
     ...
     * 获取市场的交易记录（带分页和过滤）
     */
    fastify.get('/markets/:slug/trades', {
        schema: {
            params: {
                type: 'object',
                properties: {
        ...
        const { slug } = request.params;
        const { limit = 100, cursor = 0, fromBlock, toBlock, side, outcome, } = request.query;
        // 查找市场
        const market = store.getMarketBySlug(db, slug);
        if (!market) {
            return reply.code(404).send({
        ...
        const response = {
            trades,
            total: result.total,
            nextCursor: cursor + trades.length < result.total
     ...
     * 按 TokenId 获取交易记录
     */
    fastify.get('/tokens/:tokenId/trades', {
        schema: {
            params: {
                type: 'object',
                properties: {
