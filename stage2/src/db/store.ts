 ...
 * 插入或更新事件
 */
export function upsertEvent(db: Database.Database, event: EventInsert): number {
...
export function getEventBySlug(
  ...
  return (stmt.get(slug) as EventRecord) ?? null;
}
 ...
 * 插入或更新市场
 */
export function upsertMarket(db: Database.Database, market: MarketInsert): number {
  const stmt = db.prepare(`
    INSERT INTO markets (
...
export function getMarketBySlug(
  ...
  return (stmt.get(slug) as MarketRecord) ?? null;
}
...
export function findMarketByTokenId(
  ...
  const stmt = db.prepare(`
    ...
    LIMIT 1
  `);
  return (stmt.get(tokenId, tokenId) as MarketRecord) ?? null;
}

/**
 * 根据 conditionId 获取市场
 */
export function getMarketByConditionId(
  db: Database.Database,
  conditionId: string
): MarketRecord | null {
  const stmt = db.prepare('SELECT * FROM markets WHERE condition_id = ?');
  return (stmt.get(conditionId) as MarketRecord) ?? null;
}

/**
 * 获取事件下的所有市场
 */
export function getMarketsByEventId(
  db: Database.Database,
  eventId: number
): MarketRecord[] {
  const stmt = db.prepare(`
  ...
  `);
  return stmt.all(eventId) as MarketRecord[];
}

/**
 * 获取所有市场
 */
export function getAllMarkets(db: Database.Database): MarketRecord[] {
  const stmt = db.prepare('SELECT * FROM markets ORDER BY created_at DESC');
  return stmt.all() as MarketRecord[];
}
 ...
 * 插入单个交易（幂等）
 */
export function insertTrade(db: Database.Database, trade: TradeInsert): number | null {
  const stmt = db.prepare(`
...
export function insertTrades(
  ...
  const insertMany = db.transaction((trades: TradeInsert[]) => {
    let inserted = 0;
...
export function getTradesForMarket(
  ...
  const dataStmt = db.prepare(`
    ...
    LIMIT ? OFFSET ?
  `);
  const trades = dataStmt.all(...params, limit, offset) as TradeRecord[];
 ...
 * 根据 tokenId 获取交易记录
 */
export function getTradesByTokenId(
  db: Database.Database,
  tokenId: string,
  filters: TradeFilters = {}
): TradesQueryResult {
 ...
 * 统计市场的交易数量
 */
export function getTradeCountForMarket(
  db: Database.Database,
  marketId: number
): number {
  const stmt = db.prepare('SELECT COUNT(*) as count FROM trades WHERE market_id = ?');
  const { count } = stmt.get(marketId) as { count: number };
  return count;
}
...
export function getSyncState(
  ...
  return (stmt.get(key) as SyncStateRecord) ?? null;
}
 ...
 * 删除同步状态
 */
export function deleteSyncState(db: Database.Database, key: string): void {
