...
export function decodeMarket(
  params: MarketDecodeParams,
  collateralToken: Address = USDC_ADDRESS
): MarketDecoded {
  // 验证输入参数
 ...
 * 验证市场参数
 */
function validateMarketParams(params: MarketDecodeParams): void {
  const { conditionId, questionId, oracle } = params;
 ...
 * （辅助函数，用于调试和验证）
 */
export function getCollectionIdFromTokenId(
  tokenId: Hex,
  collateralToken: Address = USDC_ADDRESS
): string {
