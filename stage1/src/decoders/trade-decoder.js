 ...
 * ```
 */
export async function decodeTrade(txHash) {
    // 验证交易哈希格式
    ...
    const orderFilledLogs = receipt.logs.filter((log) => {
        ...
        return EXCHANGE_ADDRESSES.some((addr) => addr.toLowerCase() === log.address.toLowerCase());
    });
 ...
 * @returns 所有交易的解码结果
 */
export async function decodeTradesBatch(txHashes) {
    const results = await Promise.allSettled(txHashes.map((hash) => decodeTrade(hash)));
    const allTrades = [];
    const errors = [];
...
//# sourceMappingURL=trade-decoder.js.map